...
class IntegridadeDadosTest(TestCase):
    ...
    def test_constraint_preco_decimal(self):
        ...
        servico_decimal = Servico.objects.create(
            nome="Serviço Decimal",
            duracao=30,
            preco=Decimal('25.123456789')
        )
...
class ConcorrenciaTest(TransactionTestCase):
        ...
        def criar_cliente(numero):
            try:
                cliente = Cliente.objects.create(
                    nome=f"Cliente {numero}",
                    telefone=f"1199999999{numero}"
                )
    ...
    def test_delecao_concorrente(self):
        ...
        for agendamento in agendamentos:
            thread = threading.Thread(target=deletar_agendamento, args=(agendamento.id,))
            threads.append(thread)
            thread.start()
...
class ConsultasComplexasTest(TestCase):
    ...
    def test_consulta_agregacao_clientes(self):
        ...
        agendamentos_por_cliente = Agendamento.objects.values('cliente__nome').annotate(
            total=Count('id'),
            valor_total=Sum('servico__preco')
        ).order_by('-total')
    ...
    def test_consulta_otimizada_select_related(self):
        ...
        agendamentos_com_otimizacao = list(Agendamento.objects.select_related(
            'cliente', 'servico'
        ).all()[:10])
        
        # Verificar se ambas retornam os mesmos dados
        self.assertEqual(len(agendamentos_sem_otimizacao), len(agendamentos_com_otimizacao))
    ...
    def test_consulta_otimizada_prefetch_related(self):
        ...
        clientes_com_agendamentos = list(Cliente.objects.prefetch_related(
            'agendamentos'
        ).all()[:5])
    ...
    def test_consulta_case_when(self):
        ...
        agendamentos_categorizados = Agendamento.objects.annotate(
        ...
        ).values('categoria_valor').annotate(total=Count('id'))
