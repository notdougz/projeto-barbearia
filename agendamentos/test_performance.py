...
class PerformanceViewsTest(TestCase):
    ...
    def test_painel_barbeiro_com_500_agendamentos(self):
        ...
        self.assertLess(
            response_time, 2.0, f"Painel demorou {response_time:.2f}s, esperado < 2s"
        )
    ...
    def test_lista_clientes_com_300_clientes(self):
        ...
        self.assertLess(
            response_time,
            1.0,
            f"Lista clientes demorou {response_time:.2f}s, esperado < 1s",
        )
    ...
    def test_relatorio_financeiro_mensal_performance(self):
        ...
        self.assertLess(
            response_time,
            1.0,
            f"Relatório financeiro demorou {response_time:.2f}s, esperado < 1s",
        )
    ...
    def test_agendamentos_mensais_performance(self):
        ...
        for i in range(150):
            ...
            if mes_offset == 1:
                data_agendamento = data_agendamento.replace(
                    month=data_agendamento.month - 1
                )
            elif mes_offset == 2:
                data_agendamento = data_agendamento.replace(
                    month=data_agendamento.month + 1
        ...
        self.assertLess(
            response_time,
            1.5,
            f"Agendamentos mensais demorou {response_time:.2f}s, esperado < 1.5s",
        )
...
class PerformanceQueriesTest(TestCase):
    ...
    def setUp(self):
        ...
        for i in range(100):
            cliente = Cliente.objects.create(
                nome=f"Cliente {i+1:03d}", telefone=f"11999{i:05d}"
            )
            self.clientes.append(cliente)
...
class PerformanceDatabaseTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        for i in range(10):
            servico = Servico.objects.create(
                nome=f"Serviço {i+1}", duracao=30, preco=Decimal(f"{20 + i}.00")
            )
            self.servicos.append(servico)
    ...
    def test_agregacoes_performance(self):
        ...
        receita_total = Agendamento.objects.filter(
            status="concluido", status_pagamento="pago"
        ).aggregate(total=Sum("servico__preco"), count=Count("id"))
...
class PerformanceConcurrencyTest(TransactionTestCase):
    ...
    def setUp(self):
        ...
        self.cliente = Cliente.objects.create(
            nome="Cliente Teste", telefone="11999999999"
        )
...
class PerformanceMemoryTest(TestCase):
    ...
    def test_memoria_lista_grande_clientes(self):
        ...
        for i in range(1000):
            cliente = Cliente.objects.create(
                nome=f"Cliente {i+1:04d}", telefone=f"11999{i:05d}"
            )
            clientes.append(cliente)
