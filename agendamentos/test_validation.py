...
class FormularioValidacaoTest(TestCase):
    ...
    def test_formulario_cliente_dados_maliciosos(self):
        ...
        for i, dado_malicioso in enumerate(dados_maliciosos):
            ...
            self.assertTrue(form.is_valid(), f"Dados maliciosos são aceitos pelo formulário: {dado_malicioso}")
            
            # Salvar e verificar se dados foram salvos (sem sanitização automática)
            cliente = form.save()
            self.assertEqual(cliente.nome, dado_malicioso)  # Dados são salvos como inseridos
...
class SanitizacaoValidacaoTest(TestCase):
    ...
    def test_sanitizacao_html_tags(self):
        ...
        dados_com_html = [
            ...
            ("Maria <script>alert('xss')</script>Santos", "Maria <script>alert('xss')</script>Santos"),
            ("José <img src=x onerror=alert('xss')>Silva", "José <img src=x onerror=alert('xss')>Silva"),
