...
class AutorizacaoTest(TestCase):
    ...
    def test_usuario_pode_editar_agendamento(self):
        ...
        response = self.client.post(
            reverse('editar_agendamento', args=[self.agendamento.pk]),
            agendamento_data
        )
...
class ProtecaoXSSTest(TestCase):
    ...
    def test_html_escapado_em_templates(self):
        ...
        servico = Servico.objects.create(nome="Corte", duracao=30, preco=Decimal('30.00'))
        agendamento = Agendamento.objects.create(
            cliente=cliente,
            servico=servico,
            data=date.today(),
    ...
    def test_requisicoes_sequenciais(self):
        ...
        for i in range(5):
            cliente_data = {
                ...
                'csrfmiddlewaretoken': str(self.client.get(reverse('criar_cliente')).context['csrf_token'])
            }
