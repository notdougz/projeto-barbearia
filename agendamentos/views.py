...
def criar_cliente(request):
    ...
    return render(
        request,
        "agendamentos/cliente_form.html",
...
def deletar_cliente(request, pk):
    ...
    if request.method == "POST":
        ...
        return redirect("lista_clientes")
    return render(
        request, "agendamentos/cliente_confirm_delete.html", {"cliente": cliente}
    )
...
def agendar(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f"Agendamento criado com sucesso para {agendamento.cliente.nome}!",
            )
...
def editar_agendamento(request, pk):
    ...
    return render(
        request, "agendamentos/agendar.html", {"form": form, "agendamento": agendamento}
    )
...
def deletar_agendamento(request, pk):
    ...
    return render(
        request,
        "agendamentos/agendamento_confirm_delete.html",
...
def on_the_way_agendamento(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            sms_result = smsdev_service.enviar_barbeiro_a_caminho(
                agendamento, previsao_minutos
            )

            if sms_result["sucesso"]:
                messages.success(
                    request,
                    f'Status alterado para "À caminho" e SMS enviado! Previsão: {previsao_minutos} minutos.',
                )
            else:
                messages.warning(
                    request,
                    f'Status alterado para "À caminho", mas falha no SMS: {sms_result["erro"]}',
                )
    ...
    return render(
        request,
        "agendamentos/previsao_chegada.html",
...
def concluir_agendamento(request, pk):
    ...
    agendamento.save()
    messages.success(
        request, f"Agendamento de {agendamento.cliente.nome} marcado como concluído!"
    )
    return redirect("painel_barbeiro")
...
def agendamentos_mensais(request):
    ...
    agendamentos = Agendamento.objects.filter(
        data__gte=data_inicio, data__lt=data_fim
    ).order_by("data", "hora")
    ...
    ultimo_dia = (
        ...
        if mes < 12
        else datetime(ano + 1, 1, 1) - timedelta(days=1)
    )
    ...
    pendentes = len(
        [a for a in agendamentos if a.status in ["confirmado", "a_caminho"]]
    )
...
def financeiro(request):
    ...
    filtro_pagamento = request.GET.get(
        "filtro", "todos"
    )  # todos, pendente, pago, visao_geral
    ...
    valor_pendente = sum(
        [
            a.servico.preco
            for a in todos_agendamentos.filter(status_pagamento="pendente")
        ]
    )
    valor_recebido = sum(
        [a.servico.preco for a in todos_agendamentos.filter(status_pagamento="pago")]
    )
    ...
    agendamentos_mes = Agendamento.objects.filter(
        data__gte=data_inicio_mes, data__lt=data_fim_mes
    )

    total_mes = sum([a.servico.preco for a in agendamentos_mes])
    recebido_mes = sum(
        [a.servico.preco for a in agendamentos_mes.filter(status_pagamento="pago")]
    )
    pendente_mes = sum(
        [a.servico.preco for a in agendamentos_mes.filter(status_pagamento="pendente")]
    )
    ...
    agendamentos_ano = Agendamento.objects.filter(
        data__gte=data_inicio_ano, data__lt=data_fim_ano
    )

    total_ano = sum([a.servico.preco for a in agendamentos_ano])
    recebido_ano = sum(
        [a.servico.preco for a in agendamentos_ano.filter(status_pagamento="pago")]
    )
    pendente_ano = sum(
        [a.servico.preco for a in agendamentos_ano.filter(status_pagamento="pendente")]
    )
    ...
    cortes_pagos_mes = agendamentos_mes.filter(status_pagamento="pago").order_by(
        "-data", "hora"
    )
    cortes_pendentes_mes = agendamentos_mes.filter(
        status_pagamento="pendente"
    ).order_by("-data", "hora")

    # Buscar cortes detalhados anuais
    cortes_pagos_ano = agendamentos_ano.filter(status_pagamento="pago").order_by(
        "-data", "hora"
    )
    cortes_pendentes_ano = agendamentos_ano.filter(
        status_pagamento="pendente"
    ).order_by("-data", "hora")
...
def alterar_status_pagamento(request, pk):
    ...
    if agendamento.status_pagamento == "pendente":
        ...
        messages.success(
            request, f"Pagamento de {agendamento.cliente.nome} marcado como PAGO!"
        )
    else:
        agendamento.status_pagamento = "pendente"
        messages.success(
            request, f"Pagamento de {agendamento.cliente.nome} marcado como PENDENTE!"
        )
...
def criar_servico(request):
    ...
    return render(
        request,
        "agendamentos/servico_form.html",
        {"form": form, "titulo": "Novo Serviço", "botao_texto": "Criar Serviço"},
    )
...
def editar_servico(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            servico = form.save()
            messages.success(
                request, f'Serviço "{servico.nome}" atualizado com sucesso!'
            )
            return redirect("lista_servicos")
    else:
...
def deletar_servico(request, pk):
    ...
    if request.method == "POST":
        if agendamentos_count > 0:
            messages.error(
                request,
                f'Não é possível deletar o serviço "{servico.nome}" pois ele está sendo usado em {agendamentos_count} agendamento(s).',
            )
            return redirect("lista_servicos")
    ...
    return render(
        request,
        "agendamentos/servico_confirm_delete.html",
        {"servico": servico, "agendamentos_count": agendamentos_count},
    )
